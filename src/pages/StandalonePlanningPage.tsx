...
import { Archive, BookOpen, Check, FileText, Plus, RefreshCw, Save, Upload, X } from 'lucide-react';
            ...
            <>
              <section className="grid gap-4 lg:grid-cols-[260px_1fr_320px]">
                ...
                <div className="rounded-2xl border border-zinc-200 bg-white p-5">
                  <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">Plan editor</p><p className="mt-1 text-sm text-zinc-500">Markdown remains portable and revisioned.</p></div><button disabled={!selectedPlan || busy} onClick={() => void savePlan()} className="inline-flex items-center gap-2 rounded-lg bg-zinc-950 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"><Save className="h-3.5 w-3.5" /> Save</button></div>
                  {selectedPlan ? <div className="mt-4 space-y-3"><input value={planTitle} onChange={(event) => setPlanTitle(event.target.value)} className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-lg font-bold outline-none focus:border-zinc-500" /><textarea value={planContent} onChange={(event) => setPlanContent(event.target.value)} rows={16} className="w-full resize-y rounded-lg border border-zinc-200 px-3 py-3 font-mono text-sm leading-6 outline-none focus:border-zinc-500" /></div> : <p className="mt-8 text-sm text-zinc-500">Create a plan to start drafting.</p>}
                </div>
